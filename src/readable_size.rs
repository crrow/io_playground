...
impl ReadableSize {
	pub const fn bytes(count: u64) -> ReadableSize { ReadableSize(count) }
	...
	pub const fn tb(count: u64) -> ReadableSize { ReadableSize(count * TIB) }

	pub const fn pb(count: u64) -> ReadableSize { ReadableSize(count * PIB) }

	pub const fn as_kb(self) -> u64 { self.0 / KIB }

	pub const fn as_mb(self) -> u64 { self.0 / MIB }

	pub const fn as_gb(self) -> u64 { self.0 / GIB }

	pub const fn as_tb(self) -> u64 { self.0 / TIB }

	pub const fn as_pb(self) -> u64 { self.0 / PIB }

	pub const fn as_bytes(&self) -> u64 { self.0 }

	pub const fn to_bytes(self) -> u64 { self.0 }
...
impl Add<ReadableSize> for ReadableSize {
	...
	fn add(self, rhs: ReadableSize) -> Self::Output { ReadableSize(self.0 + rhs.0) }
}

impl Sub<ReadableSize> for ReadableSize {
	type Output = ReadableSize;

	fn sub(self, rhs: ReadableSize) -> Self::Output { ReadableSize(self.0 - rhs.0) }
}

// impl AsRef<u64> for ReadableSize {
//     fn as_ref(&self) -> &u64 { &self.0 }
// }
//
// impl<T: AsRef<u64>> Rem<T> for ReadableSize {
//     type Output = u64;
//
//     fn rem(self, rhs: T) -> Self::Output { self.as_bytes() % rhs.as_ref() }
// }
